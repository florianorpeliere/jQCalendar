...
(function (factory) {
   ...
   $.extend(Datepicker.prototype, {
      ...
      //TODO
      _hover: function (id, month, year, td) {
         var inst,
            target = $(id);
         ...
         if (inst.settings.rangeSelection) {
            inst.hoveredDay = inst.currentDay = $("a", td).html();
            inst.hoveredMonth = inst.currentMonth = month;
            inst.hoveredYear = inst.currentYear = year;
            if (inst.settings.rangeSelection === "from") {
               inst.settings.beginSelection = this._formatDate(inst, inst.currentDay, inst.currentMonth, inst.currentYear);
            }
      ...
      _attachHandlers: function (inst) {
         ...
         //TODO add handler
         var that = this;
         inst.dpDiv.find("[data-over]").map(function () {
            var handler = {
               hover: function () {
                  //Empecher le mouseenter de se fired à chaque fois puisque le calendrier est recréé à chaque fois
                  if (that.lastThis === null || (that.lastThis && that.lastThis.innerHTML !== this.innerHTML)) {
                     that.lastThis = this;
                     $.datepicker._hover(id, +this.getAttribute("data-month"), +this.getAttribute("data-year"), this);
                  }
                  return false;
               }
            };
            $(this).bind("mouseenter", handler[this.getAttribute("data-over")]);
         });
         ...
         inst.dpDiv.find("[data-handler]").map(function () {
            var handler = {
               ...
               selectDay: function () {
                  ...
                  //TODO
                  inst.dpDiv.find("[data-over]").unbind('mouseenter');
                  return false;
               },
               ...
               for (dRow = 0; dRow < numRows; dRow++) { // create date picker rows
                  ...
                  for (dow = 0; dow < 7; dow++) { // create date picker days
                     ...
                     tbody += "<td class='" +
                        ...
                        (unselectable ? "" : " data-over='hover' data-handler='selectDay' data-event='click' data-month='" + printDate.getMonth() + "' data-year='" + printDate.getFullYear() + "'") + ">" + // actions
